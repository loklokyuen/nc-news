...
export default function Article() {
	...
	if (message)
		return (
			<section
				className={`m-2 font-semibold bg-mandys-pink-50/50 p-4 max-w-xl mx-auto ${isError ? "text-feedback-error" : "text-primary"}`}
			>
				{message}
			...
			<section className="max-w-4xl bg-surface text-neutral mt-2 justify-center flex flex-col self-center">
				...
				<section className="relative">
					<img
						...
						alt="article image"
						className="p-2 w-80vw"
					/>
					<NavLink
						to={`/articles?topic=${article.topic}`}
						className="absolute bottom-3 right-3 p-0 bg-primary/70 rounded-sm"
					>
						<p className="m-1 p-0.5 text-white italic hover:text-secondary-light">
