...
export default function ArticleItem({ article }) {
	...
	return (
		<li ref={articleRef} className={`article-item ${loaded ? "loaded" : ""}`}>
			...
			<div className="relative">
				<NavLink to={"/articles/" + article.article_id}>
					<img
						...
						alt="article image"
						className="rounded-lg article-item-img px-1 py-1 max-h-40 object-cover"
					/>
				</NavLink>
				<NavLink
					to={`/articles?topic=${article.topic}`}
					className="absolute bottom-1.5 right-2.5 p-0 bg-primary/70 rounded-sm"
				>
