...
export default function NewTopic({ setCreateNewTopic, onTopicCreated }) {
		...
		postTopic(slug, description)
			...
			.catch((err) => {
				...
				const errorMessage =
					...
					"Something went wrong! Unable to add new topic, please try again later.";
	...
	if (successMessage)
		return (
			<section className="p-4 m-2 text-highland-500">{successMessage}</section>
		);
	return (
		<section className="flex flex-col items-center rounded-2xl bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
			...
			<div className="grid grid-cols-3 w-full">
				...
				<input
					...
					type="text"
					value={slug}
					className="p-1 m-1 border-b-1 text-tertiary col-span-2 "
					onChange={(e) => {
			...
			{errorMessage && (
				<div className={`message max-w-3xl text-feedback-error `}>
